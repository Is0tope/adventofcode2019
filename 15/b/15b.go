...
func main() {
	...
	fmt.Println("MAZE FULLY MAPPED")
	printGrid(OXYGEN, 40)

	// Set up new BFS for oxygen
	QUEUE2 := make([]oxygen, 0)
	SEEN := make(map[point]struct{})
	QUEUE2 = append(QUEUE2, oxygen{OXYGEN, 0})
	maxDistance := int64(0)
	for len(QUEUE2) > 0 {
		// pop off a point
		o := QUEUE2[0]
		QUEUE2 = QUEUE2[1:]
		// if we have seen this before, skip
		if _, seen := SEEN[o.position]; seen {
			continue
		}
		// Update max distance
		if o.distance > maxDistance {
			maxDistance = o.distance
		}
		// Update the SEEN
		SEEN[o.position] = struct{}{}

		// Select directions (we push back the direction we came as it has been seem already anyway)
		states := make([]oxygen, 0)
		north := o.position
		north.y++
		if getTile(north) != "#" {
			states = append(states, oxygen{north, o.distance + 1})
		}
		south := o.position
		south.y--
		if getTile(south) != "#" {
			states = append(states, oxygen{south, o.distance + 1})
		}
		west := o.position
		west.x--
		if getTile(west) != "#" {
			states = append(states, oxygen{west, o.distance + 1})
		}
		east := o.position
		east.x++
		if getTile(east) != "#" {
			states = append(states, oxygen{east, o.distance + 1})
		}
		// Add to queue
		QUEUE2 = append(QUEUE2, states...)
	}
	fmt.Println("Time taken for oxygen to fill:", maxDistance)
