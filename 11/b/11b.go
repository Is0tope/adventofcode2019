...
func (r *robot) run() {
	...
	for <-r.brain.commchan {
		...
		r.turn(int(turn))
		r.step()
		fmt.Println("at", p, "angle", r.angle, "current color:", currentcol, "new color:", newcol, "turn", turn, "new position", r.x, r.y)
	}
}
...
func main() {
	...
	rob := NewRobot(code)
	// change the first panel to a white one
	rob.grid[point{0, 0}] = 1
	rob.run()
	// Draw the image
	for i := 2; i > -6; i-- {
		for j := 0; j < 80; j++ {
			col, ok := rob.grid[point{int64(j), int64(i)}]
			char := " "
			if ok && col == 1 {
				char = "#"
			}
